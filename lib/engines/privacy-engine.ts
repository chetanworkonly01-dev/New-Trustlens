...
export async function runPrivacyAudit(
  ...
  const log = (testId: string, status: string, message: string, methodology?: string, phase?: string) => {
    onProgress?.({ timestamp: new Date().toISOString(), testId, testName: '', wcag: '', status: status as any, message, pillar: 'privacy', methodology, phase });
  };
  ...
  for (const pageData of pages) {
    ...
    try {
      ...
      log('PRIV-CK', 'running', '━━━ Layer A: Cookie & Storage Analysis', ' Art. 5(1)(e) + ePrivacy Directive', 'Layer A: Cookie Audit');
      ...
      log('PRIV-CK-EX', 'running', '  → Cookie Lifetime: Flagging non-essential cookies persisting >1 year (CNIL guidance)', ' Art. 5(1)(e) — Storage Limitation', 'Layer A: Cookie Audit');
      log('PRIV-CK-SC', 'running', '  → Cookie Security: Secure flag, HttpOnly, SameSite attributes on sensitive cookies', ' Art. 32 — Security of Processing', 'Layer A: Cookie Audit');
      log('PRIV-CK-ST', 'running', '  → Client Storage: localStorage / sessionStorage item count (data minimisation check)', ' Art. 5(1)(c) — Data Minimisation', 'Layer A: Cookie Audit');
      ...
      if (longCookies.length > 0) {
        findings.push({
          ...
          pageUrl: pageData.url, severity: 'medium', regulation: ['', 'ePrivacy'],
      ...
      log('PRIV-CK', 'pass', `  ✓ Layer A complete — Cookies: ${cookies.length} | Storage: ${storageAnalysis.ls + storageAnalysis.ss} items`, ' Art. 5 + ePrivacy', 'Layer A: Cookie Audit');

      // ── Layer B: Tracker Detection ──
      log('PRIV-TR', 'running', '━━━ Layer B: Third-Party Tracker Detection', ' Art. 6 — Lawful Basis for Processing', 'Layer B: Tracker Detection');
      log('PRIV-TR-TK', 'running', '  → Known Trackers: Google Analytics, Meta Pixel, Hotjar, Clarity, TikTok Pixel detection', ' Art. 6 — Consent Required for Trackers', 'Layer B: Tracker Detection');
      log('PRIV-TR-FP', 'running', '  → Browser Fingerprinting: Canvas, AudioContext, font-enumeration fingerprint signals', ' Art. 4(1) — Fingerprint = Personal Data', 'Layer B: Tracker Detection');
      ...
      if (fingerprinting.length > 0) {
        findings.push({
          ...
          pageUrl: pageData.url, severity: 'high', regulation: ['', 'IN-DPDPA', 'ePrivacy'],
      ...
      log('PRIV-TR', 'pass', `  ✓ Layer B complete — Fingerprinting: ${fingerprinting.length} signals | Pixels: ${trackingPixels.length}`, ' Art. 6', 'Layer B: Tracker Detection');

      // ── Layer C: Consent Infrastructure ──
      log('PRIV-CN', 'running', '━━━ Layer C: Consent Infrastructure Audit', ' Art. 7 + ICO Enforcement Guidance', 'Layer C: Consent Audit');
      log('PRIV-CN-BN', 'running', '  → Consent Banner: Detection of cookie/ consent UI presence', ' Art. 7 — Conditions for Consent', 'Layer C: Consent Audit');
      log('PRIV-CN-RJ', 'running', '  → Reject Option: "Reject All" / "Decline" button at first layer (ICO, CNIL requirement)', ' Art. 7 + CNIL Guidance — Equal Prominence', 'Layer C: Consent Audit');
      ...
      log('PRIV-CN-LI', 'running', '  → Legitimate Interest: Pre-selected LI toggles without user action (illegal under )', ' Art. 6(1)(f) — Legitimate Interest', 'Layer C: Consent Audit');
      log('PRIV-CN-CW', 'running', '  → Consent Wall: Page access blocked unless user consents (generally illegal )', ' Art. 7(4) — Conditional Consent', 'Layer C: Consent Audit');
      ...
      if (hasConsentBanner) {
        const hasRejectOption = await page.evaluate(() => {
          const banners = document.querySelectorAll('[class*="cookie"], [class*="consent"], [class*=""]');
        ...
        if (!hasRejectOption) {
          findings.push({
            ...
            pageUrl: pageData.url, severity: 'critical', regulation: ['', 'IN-DPDPA', 'ePrivacy'],
      ...
      log('PRIV-CN', hasConsentBanner ? 'pass' : 'fail', `  ✓ Layer C complete — Consent banner: ${hasConsentBanner ? 'Found ✓' : 'MISSING ✗'}`, ' Art. 7 + ICO', 'Layer C: Consent Audit');

      // ── Layer D: Data Collection Analysis ──
      log('PRIV-DC', 'running', '━━━ Layer D: Data Collection & Minimisation Analysis', ' Art. 5(1)(c) — Data Minimisation Principle', 'Layer D: Data Collection');
      log('PRIV-DC-FM', 'running', '  → Form Field Audit: Counting visible fields — email + phone + >6 fields = excessive', ' Art. 5(1)(c) — Collect Only What Is Necessary', 'Layer D: Data Collection');
      log('PRIV-DC-HF', 'running', '  → Hidden Input Fields: Tracking submission of data user cannot see or control', ' Art. 5(1)(a) — Transparency Principle', 'Layer D: Data Collection');
      log('PRIV-DC-SL', 'running', '  → Social Login: Data-sharing disclosure for OAuth providers (Google, Facebook, Apple)', ' Art. 13 — Information to Be Provided', 'Layer D: Data Collection');
      ...
      for (const form of formAnalysis) {
        if (form.hasEmail && form.hasPhone && form.fieldCount >= 6) {
          findings.push({
            ...
            pageUrl: pageData.url, severity: 'medium', regulation: ['', 'IN-DPDPA'],
      ...
      log('PRIV-DC', 'pass', `  ✓ Layer D complete — Forms analysed: ${formAnalysis.length}`, ' Art. 5(1)(c)', 'Layer D: Data Collection');

      // ── Layer E: Privacy Policy Analysis ──
      log('PRIV-PP', 'running', '━━━ Layer E: Policy Quality Audit', ' Art. 12–14 — Transparency Obligations', 'Layer E: Policy');
      log('PRIV-PP-LK', 'running', '  → Policy Link: Visible link from homepage/footer (legal requirement, Art. 13)', ' Art. 13 — Information at Collection',);
      log('PRIV-PP-LB', 'running', '  → Legal Basis: Each processing purpose must state its lawful basis (Art. 6)', ' Art. 6 — Lawful Basis Per Purpose',);
      log('PRIV-PP-PL', 'running', '  → Plain Language: Policy must be written in clear, plain language (Art. 12)', ' Art. 12 — Transparent Communication',);
      log('PRIV-PP-RT', 'running', '  → User Rights: Access, erasure, portability, objection rights must be explained', ' Art. 15–22 — Data Subject Rights',);
      log('PRIV-PP-DP', 'running', '  → DPO Contact: Data Protection Officer contact details required (Art. 37)', ' Art. 37 — DPO Designation',);
      ...
      log('PRIV-PP', hasPrivacyPolicy ? 'pass' : 'fail', `  ✓ Layer E complete —  : ${hasPrivacyPolicy ? 'Found ✓' : 'MISSING ✗'}`, ' Art. 13/14',);

      // ── Layer F: Security Baseline ──
      log('PRIV-SC', 'running', '━━━ Layer F: Security Baseline & Data Protection', ' Art. 32 — Security of Processing', 'Layer F: Security');
      log('PRIV-SC-MC', 'running', '  → Mixed Content: HTTP resources on HTTPS page (data interception risk)', ' Art. 32 — Encryption in Transit', 'Layer F: Security');
      log('PRIV-SC-HS', 'running', '  → Security Headers: HSTS, CSP, X-Frame-Options, Referrer-Policy presence', ' Art. 32 + OWASP Security Headers', 'Layer F: Security');
      log('PRIV-SC-HT', 'running', '  → HTTPS Enforcement: All resources served over TLS (Strict-Transport-Security)', ' Art. 32(1)(a) — Pseudonymisation & Encryption', 'Layer F: Security');
      // Mixed content
      ...
      if (mixedContent.length > 0) {
        ...
        findings.push({
          ...
          pageUrl: pageData.url, severity: 'high', regulation: [''],
      ...
      log('PRIV-SC', mixedContent.length > 0 ? 'fail' : 'pass', `  ✓ Layer F complete — Mixed content: ${mixedContent.length} | Security headers checked`, ' Art. 32', 'Layer F: Security');
  ...
  if (!hasConsentBanner && trackers.length > 0) {
    findings.push({
      ...
      pageUrl: pages[0]?.url || '', severity: 'critical', regulation: ['', 'ePrivacy', 'CCPA', 'IN-DPDPA'],
  ...
  if (!hasPrivacyPolicy) {
    findings.push({
      ...
      pageUrl: pages[0]?.url || '', severity: 'high', regulation: ['', 'CCPA', 'IN-DPDPA'],
  ...
  log('PRIV-REG', 'running', '━━━ Layer G: Multi-Regulation Risk Mapping', ' Regulatory Framework', 'Layer G: Regulatory Mapping');
  log('PRIV-REG-EU', 'running', '  →  (EU 2016/679): Data protection, consent, data subject rights', ' — European Data Protection Regulation', 'Layer G: Regulatory Mapping');
  log('PRIV-REG-US', 'running', '  → CCPA / CPRA: California consumer privacy rights & opt-out of sale', 'CCPA / CPRA — California Rights', 'Layer G: Regulatory Mapping');
  log('PRIV-REG-UK', 'running', '  → UK  + PECR: Post-Brexit UK data protection & cookie rules (ICO)', 'UK  + PECR — ICO Enforcement', 'Layer G: Regulatory Mapping');
  ...
  log('PRIV-REG-BD', 'running', '  → By Design: Ann Cavoukian 7 Principles — proactive, default, embedded', 'by Design — ISO 29101 Framework', 'Layer G: Regulatory Mapping');
  const regSet = new Set<string>();
  for (const f of findings) f.regulation.forEach(r => regSet.add(r));
  log('PRIV-REG', 'pass', `  ✓ Layer G complete — Mapped to ${regSet.size} regulation(s): ${[...regSet].join(', ')}`, ' Regulatory Framework', 'Layer G: Regulatory Mapping');
  // Build result
